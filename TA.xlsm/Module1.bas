...
Public Sub RefreshMondayExec()
Dim outputRange As Range
    Set outputRange = Application.Run("VBAUtils.xlsm!HTTPDownloadFile", _
                "http://www.veloxfintech.com/datafiles/Monday/5555786972.txt", _
                ActiveWorkbook, _
                "", "", 2500, "start-of-day", "Monday Data", False, 1, 30000)
...
End Sub
Public Sub LoadAppointmentsExec(queryStartDate As String, queryEndDate As String, mailboxOwner As String)
    ...
    Set objOwner = olNS.CreateRecipient(mailboxOwner)
    objOwner.Resolve
    ...
    datStartUTC = oPA.LocalTimeToUTC(queryStartDate)
    datEndUTC = oPA.LocalTimeToUTC(queryEndDate)
    ...
    Dim myArr() As Variant: ReDim myArr(0 To 15, 0 To olColRestrictedItems.Count - 1)
    ...
    Set resultRange = ws.Range(Cells(3, 1), Cells(queryNumResultsRange.value, 16))
    ...
    ws.Range("A2:P2").Value2 = Array("Subject", "Categories", "Duration", "Recurrence", "Location", "Start", "End", "Start Hour", _
            "Start Minute", "End Hour", "End Minute", "Start Year", "Status", "Recipients", "Organizer", "Item ID")
...
cleanExit:
    ...
    Set resultRange = ws.Range(Cells(startRow - 1, 1), Cells(lastEndRow, 16))
